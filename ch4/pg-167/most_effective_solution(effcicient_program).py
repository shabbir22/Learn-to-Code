...
#Alternative of iteration: we don't need to iterate over the whose list of scores as we are only looking for the highest score;
                           # as a result, we can only search in the 'solutions_with_high_score' list


cost=100.0         
most_effective=0    
for i in range(len(solutions_with_high_score)):   #only iterating over the 'solutions_with_high_score' list
    index=solutions_with_high_score[i]  #we are using the indices in solution_with_high_score list as our indices in this case
    if costs[index]<cost:             #If the cost of that index from the costs list is less than the previous item
